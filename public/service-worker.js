...
/**
 * App-shell service worker for Guitar Practice Companion.
 *
 ...
 * `gpc:service-worker-manifest` plugin in vite.config.ts, which fills in the
...
const CACHE_NAME = `gpc-shell-${BUILD_MANIFEST.version}`;
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        names
          .filter((name) => name.startsWith('gpc-shell-') && name !== CACHE_NAME)
