 ...
 * of the app never imports `db` directly, so when we add /users/{uid}/skills
 * and /users/{uid}/sessions later, the collection layout stays in one file.
 *
...
export async function ensureUserProfile(user: User): Promise<void> {
  ...
  await setDoc(
    userDoc(user.uid),
    {
      ...
      ...(isNewProfile ? { createdAt: serverTimestamp(), practicePings: 0 } : {}),
...
/**
 * Bumps the demo counter. Uses `increment()` so two devices practising offline
 * both land when they reconnect, instead of one overwriting the other.
 */
export async function recordPracticePing(uid: string): Promise<void> {
  await setDoc(
    userDoc(uid),
    { practicePings: increment(1), lastLoginAt: serverTimestamp() },
    { merge: true },
  );
}
