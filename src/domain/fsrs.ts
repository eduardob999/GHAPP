 ...
 * it has no idea *why*: it cannot say how likely you are to still have a chord
 * under your fingers tomorrow, so it cannot aim for a particular chance of
 * remembering.
 *
 ...
 * `src/domain/`.
 *
 * Guitar is not flashcards, and two departures follow from that. The forgetting
 * curve is fitted to recall of facts; a motor skill decays differently and,
 * more importantly, *nobody practises a chord once a year*. So intervals are
 * capped to something a practice habit can actually contain, and the grade
 * comes from four buttons rather than FSRS's usual four ratings by coincidence
 * of arity — `fail` is a lapse, `hard`/`good`/`easy` are successes.
 */
 ...
 * player's own history says, which is the "adaptable" part — a beginner whose
 * chords decay in two days should not be scheduled on a curve fitted to
 * medical students memorising vocabulary.
 */
 ...
 *
 * FSRS will happily suggest a year once something is well known. A practice
 * companion that goes quiet for a year about your G chord has stopped being a
 * practice companion — and unlike a fact, a chord you have not played in months
 * is gone from your hands whatever the model believes about your memory.
 */
...
/**
 * Nudges the weights toward this player's own history.
 *
 * Full FSRS optimisation fits nineteen parameters by gradient descent over
 * thousands of reviews. Nobody practising guitar for seven minutes a day will
 * ever produce that, and a badly-fitted model is worse than the population
 * average. So this does the one adjustment that a few dozen reviews *can*
 * support: compare predicted recall with what actually happened, and scale the
 * stability-growth term so the schedule as a whole runs faster or slower.
 *
